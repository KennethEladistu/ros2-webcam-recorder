...
# Custom robot interfaces
CUSTOM_CAMERA_MSG_TYPES = [
    "robot_interfaces/msg/CameraData",
]

# Language instruction message types
LANGUAGE_MSG_TYPES = [
    "std_msgs/msg/String",
    "std_msgs/String",
    "robot_interfaces/msg/LanguageInstruction",
]
...
def _is_custom_camera_topic(topic: str, msgtype: str) -> bool:
    """Check if a topic contains custom camera data."""
    return msgtype in CUSTOM_CAMERA_MSG_TYPES

def _is_language_topic(topic: str, msgtype: str) -> bool:
    """Check if a topic contains language instruction."""
    return msgtype in LANGUAGE_MSG_TYPES
...
def _extract_camera_name(topic: str) -> str:
    ...
    filtered = [
        p for p in parts if p not in ["image_raw", "image", "rgb", "compressed", "rect", "msg", "camera_data"]
    ]
    if filtered:
...
class RosbagReader:
    ...
    def _analyze_metadata_yaml(self, metadata_path: Path, info: DatasetInfo) -> None:
        ...
        for topic_info in topics:
            ...
            # Detect custom camera topics
            elif _is_custom_camera_topic(topic, msgtype):
                cam_name = _extract_camera_name(topic)
                info.cameras[cam_name] = CameraInfo(
                    name=cam_name,
                    height=480,
                ...
                total_image_msgs = max(total_image_msgs, msgcount)
            # Detect language instruction topics
            elif _is_language_topic(topic, msgtype):
                info.observation_schema[topic] = FieldSchema(
                    name=topic,
                    shape=(),
                    dtype=Dtype.FLOAT32,
    ...
    def _sample_image_resolution(self, reader: Any, info: DatasetInfo) -> None:
        ...
        # Find both standard image AND custom camera connections
        image_connections = []
        for conn in reader.connections:
            if _is_image_topic(conn.topic, conn.msgtype):
                image_connections.append(conn)
            elif _is_custom_camera_topic(conn.topic, conn.msgtype):
                image_connections.append(conn)
        ...
        for conn in image_connections:
            ...
            try:
                for connection, timestamp, rawdata in reader.messages(connections=[conn]):
                    ...
                    # robot_interfaces/msg/CameraData — extract from rgb_image field
                    if _is_custom_camera_topic(connection.topic, connection.msgtype):
                        rgb = msg.rgb_image
                        encoding = getattr(rgb, "encoding", "bgr8")
                        info.cameras[cam_name] = CameraInfo(
                            name=cam_name,
                            height=rgb.height,
                            width=rgb.width,
                    ...
                    # sensor_msgs/Image
                    elif hasattr(msg, "height") and hasattr(msg, "width"):
    ...
    def read_episodes(self, path: Path) -> Iterator[Episode]:
        ...
        """
        _check_rosbags()
        # Extract language instruction from bag
        language_instruction = None
        try:
            from rosbags.highlevel import AnyReader
            with AnyReader([path]) as reader:
                language_connections = [
                    conn for conn in reader.connections if _is_language_topic(conn.topic, conn.msgtype)
                ]
                for connection, timestamp, rawdata in reader.messages(connections=language_connections):
                    msg = reader.deserialize(rawdata, connection.msgtype)
                    if hasattr(msg, "task_name"):
                        language_instruction = msg.task_name
    ...
    def _load_frames_from_bag(self, path: Path) -> Iterator[Frame]:
        ...
        with AnyReader([path]) as reader:
            # Pre-load all joint states: list of (timestamp_ns, positions_array)
            joint_states: list[tuple[int, Any]] = []
            joint_connections = [
                conn for conn in reader.connections
                if conn.msgtype in STATE_MSG_TYPES
            ]
            for connection, timestamp, rawdata in reader.messages(connections=joint_connections):
                msg = reader.deserialize(rawdata, connection.msgtype)
                if hasattr(msg, "position") and len(msg.position) > 0:
                    joint_states.append((timestamp, np.array(list(msg.position), dtype=np.float32)))

            # Find both standard image AND custom camera connections
            image_connections = [
                conn for conn in reader.connections
                if _is_image_topic(conn.topic, conn.msgtype)
                or _is_custom_camera_topic(conn.topic, conn.msgtype)
            ]
            ...
            for connection, timestamp, rawdata in reader.messages(connections=image_connections):
                ...
                # robot_interfaces/msg/CameraData — use rgb_image field
                if _is_custom_camera_topic(connection.topic, connection.msgtype):
                    rgb = msg.rgb_image
                ...
                # sensor_msgs/Image — original handling
                ...
                state = None
                if joint_states:
                    closest = min(joint_states, key=lambda x: abs(x[0] - timestamp))
                    state = closest[1]
